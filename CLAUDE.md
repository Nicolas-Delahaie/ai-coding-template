...
- **`template/`** — le template distribué. Tout ce qui s'y trouve (`.ai/`, `.claude/`, `backlog/`, `CLAUDE.md`) est copié tel quel dans les projets cibles par `apply.sh`. C'est le produit.
...
`apply.sh` télécharge le dépôt depuis GitHub puis copie le contenu de `template/` à la racine du projet courant. Toute modif de la structure de `template/` est prise en compte automatiquement — pas de liste à maintenir.
